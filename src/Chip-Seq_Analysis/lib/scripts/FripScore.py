...
def computefripscore(saffile, bamfile, outputtextfile):
    ...
    shell(f"featureCounts -p -F SAF -a {saffile} -o {outputtextfile} {bamfile}")
...
for bamFile, outputTextFile, saffile in zip(
    snakemake.input.bamFiles, snakemake.output, 
    snakemake.input.safFiles):

    pool.submit(computefripscore, saffile, bamFile, outputTextFile)
